...
void LensSystemModel::calculate()
{
    ...
    for (const auto current_lens : lens_sorted)
    {
        ...
        if (current_object->get_x_coordinate() > current_lens.get_x_coordinate())
        {
            Lens auxilary_lens = Lens(current_lens.get_x_coordinate(), -current_lens.get_focal_length());
            current_image = auxilary_lens.calculateImage(*current_object);
        }
        else
        {
...
{
    const float x_coordinate_difference = x_coordinate - in_object.get_x_coordinate();
    const float distance_sign = x_coordinate_difference < 0 ? -1.0f : 1.0f;
    const float distance_to_object = std::abs(x_coordinate_difference);
    const float distance_to_image = 1.0f / (1.0f / focal_length - 1.0f / distance_to_object);
    const float size_scale = -(distance_to_image / distance_to_object);

    float image_x_coordinate = x_coordinate + distance_sign * distance_to_image;
